...
export const completeTask = async (req, res) => {
    try {
        ...
        if (task.quiz && task.quiz.length > 0) {
            ...
            const allCorrect = task.quiz.every((question, index) => quizAnswers[index] === question.answer);
            if (!allCorrect) {
                return res.status(400).json({
                    success: false,
        ...
        for (const skillReward of task.skillRewards) {
            if (!skillReward.skill || !skillReward.points)
                continue;
        ...
        for (const skill of masteredSkills) {
            ...
            const existingAchievement = user.profileData.achievements.find(a => a.title === `${skill.name} Mastery` && a.icon === 'skill-mastery');
