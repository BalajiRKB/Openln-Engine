  ...
  const skillCategories: Record<string, Skill[]> = {
    Technical: skills.filter((s) =>
      [
        ...
        "DevOps",
        "Coding",
      ].some((term) => s.name.includes(term)),
    ),
    Business: skills.filter((s) =>
      ["Business Planning", "Market Research", "Pitching", "Marketing"].some(
        (term) => s.name.includes(term),
      ),
    ),
    "Problem Solving": skills.filter((s) =>
      ["Problem Solving", "Algorithms", "Data Structures"].some((term) =>
        s.name.includes(term),
      ),
    ),
    Communication: skills.filter((s) =>
      ["Interview Preparation", "Documentation", "Writing"].some((term) =>
        s.name.includes(term),
      ),
    ),
  };

  const getCategoryAverage = (category: Skill[]): number => {
    if (category.length === 0) return 0;
    return Math.floor(
      category.reduce((sum, skill) => sum + skill.proficiency, 0) /
        category.length,
    );
  };
            ...
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <p className={`text-3xl font-bold ${rankColors[rank]}`}>
                    {rank}
                  </p>
                </div>
                <div>
              ...
              <div className="mt-6">
                ...
                <p className="text-lg font-medium bg-purple-900/30 rounded-lg px-4 py-2">
                  {goal}
                </p>
              </div>
            </div>
                    ...
                    <div key={category}>
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="text-gray-400">
                          {getCategoryAverage(categorySkills)}%
                        </span>
                      </div>
                      <div className="w-full h-3 bg-gray-700 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-purple-600 rounded-full"
                          style={{
                            width: `${getCategoryAverage(categorySkills)}%`,
                          }}
                        ></div>
                      </div>
                    </div>
            ...
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-6 md:col-span-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {achievements?.slice(0, 6).map((achievement, index) => (
                  <div
                    key={index}
                    className="bg-gray-800/50 rounded-lg p-4 flex gap-3"
                  >
            ...
            {skills.length === 0 ? (
              <div className="bg-gray-900 rounded-xl p-8 text-center">
                <p className="text-gray-400">
                  Complete tasks to develop skills!
                </p>
              </div>
            ) : (
